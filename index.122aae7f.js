...
t=tN(t),!r&&e?"["+t+"]":t)}).join(r?".":""):e}let tP=k.toFlatObject(k,{},null,function(t){return/^is[A-Z]/.test(t)});var t_=/**
...
r=tN(r),a.forEach(function(t,n){k.isUndefined(t)||null===t||e.append(!0===s?tL([r],n,i):null===s?r:r+"[]",u(t))}),!1)}}return!!tx(t)||(e.append(tL(o,r,i),u(t)),!1)}let c=[],h=Object.assign(tP,{defaultVisitor:l,convertValue:u,isVisitable:tx});if(!k.isObject(t))throw TypeError("data must be an object");return!function t(r,n){if(!k.isUndefined(r)){if(-1!==c.indexOf(r))throw Error("Circular reference detected in "+n.join("."));c.push(r),k.forEach(r,function(r,i){let s=!(k.isUndefined(r)||null===r)&&o.call(e,r,k.isString(i)?i.trim():i,n,h);!0===s&&t(r,n?n.concat(i):[i])}),c.pop()}}(t),e};/**
...
f.send(o||null)})};let ei={http:null,xhr:eo};k.forEach(ei,(t,e)=>{if(t){try{Object.defineProperty(t,"name",{value:e})}catch(t){// eslint-disable-next-line no-empty
 ...
 *
 * @returns {object}
 */function(t,e,r){if("object"!=typeof t)throw new M("options must be an object",M.ERR_BAD_OPTION_VALUE);let n=Object.keys(t),o=n.length;for(;o-- >0;){let i=n[o],s=e[i];if(s){let e=t[i],r=void 0===e||s(e,i,t);if(!0!==r)throw new M("option "+i+" must be "+r,M.ERR_BAD_OPTION_VALUE);continue}if(!0!==r)throw new M("Unknown option "+i,M.ERR_BAD_OPTION)}},validators:eh};let ey=ed.validators;/**
...
let{Axios:ew,AxiosError:eA,CanceledError:eO,isCancel:eR,CancelToken:eB,VERSION:ev,all:eS,Cancel:eT,isAxiosError:eU,spread:eC,toFormData:eI,AxiosHeaders:ex,HttpStatusCode:eN,formToJSON:eL,getAdapter:eP,mergeConfig:e_}=eE;eE.defaults.headers.common["x-api-key"]="live_iGSvPnRg8pvnoWaV96pD0qkOsNoyBuZMZKzZwbBPoVdtkf8QV2suGCLls79FwX1C";let ej="https://api.thecatapi.com/v1",eF=document.querySelector(".breed-select"),eD=document.querySelector(".loader"),ek=document.querySelector(".error"),eM=document.querySelector(".cat-info");function e$(t){eM.innerHTML=`
    <img src="${t[0].url}" alt="Cat Image" />
    <div class="cat-wrapper">
    <h2>${t[0].breeds[0].name}</h2>
    <p> ${t[0].breeds[0].description}</p>
    <p><strong>Temperament:</strong> ${t[0].breeds[0].temperament}</p>
    </div>
  `}eE.get(`${ej}/breeds`).then(t=>t.data).catch(t=>{throw showError("Failed to fetch breeds."),t}).then(t=>{ek.style.display="none",eD.style.display="none",t.forEach(t=>{let e=document.createElement("option");e.value=t.id,e.textContent=t.name,eF.appendChild(e)}),eF.addEventListener("change",()=>{let t=eF.value;eD.style.display="block",eM.innerHTML="",eE.get(`${ej}/images/search?breed_ids=${t}`).then(t=>t.data).catch(t=>{throw showError("Failed to fetch cat information."),t}).then(e$).catch(t=>console.error(t)).finally(()=>{eD.style.display="none"})}),new SlimSelect({select:".breed-select"})}).catch(t=>console.error(t))}();//# sourceMappingURL=index.122aae7f.js.map

//# sourceMappingURL=index.122aae7f.js.map
